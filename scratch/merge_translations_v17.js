...
if (entry) {
  ...
  if (match) {
    ...
    const newTranslations = {
      ...
      // Skill descriptions: split suffixes
      ...
      " Use when the user asks to turn their workflow, interaction, or multi-step process into a skill, or when they say \"make this a skill\", \"create a skill from what we just did\", \"package this workflow\" or similar. Do not use for creating skills from scratch without an existing workflow (use a generic skill-creator for that).": "。当用户要求将他们的工作流、交互或多步骤过程转变为技能，或者他们说“将此制作成技能”、“从我们刚才的操作中创建一个技能”、“打包此工作流”或类似内容时使用。如果没有现有的工作流，请勿用于从头创建技能（为此请使用通用的技能创建器）。",
      ...
      "Orchestrates Android development tasks including project creation, deployment, SDK management, and environment diagnostics using the android command-line tool.": "使用 android 命令行工具编排 Android 开发任务，包括项目创建、部署、SDK 管理 and 环境诊断。",
      ...
      'Use when needing clinical significance, pathogenicity classifications (e.g., Pathogenic, Benign, VUS), clinical evidence rationales, or finding "hard positive" benchmark controls for human genomic variants.': "在需要临床意义、致病性分类（例如，致病、良性、VUS）、临床证据原理，或为人类基因组变异寻找“强阳性”基准对照时使用。",
      "Use when you want to look up, map, and search for short genetic variants (SNPs, indels) in NCBI's dbSNP database. Resolves between rsIDs, genomic coordinates in VCF format, and HGVS strings. For an rsID, returns variant type, gene associations, clinical significance, allele frequencies, and genomic coordinates (GRCh38).": "当您想在 NCBI 的 dbSNP 数据库中查找、映射和搜索短遗传变异（SNP、插入缺失）时使用。在 rsID、VCF 格式的基因组坐标和 HGVS 字符串之间进行解析。对于 rsID，返回变异类型、基因关联、临床意义、等位基因频率和基因组坐标 (GRCh38)。",
      ...
      "Query the OpenAlex scholarly database for research papers, authors, institutions, topics, sources, publishers, funders, geo-locations, and keywords. Use when searching academic papers, resolving DOIs, downloading open-access PDFs, finding an author's publications, aggregating bibliometric data (citation counts, h-index, impact factor), exploring the research taxonomies, or performing DOI lookups.": "在 OpenAlex 学术数据库中查询研究论文、作者、机构、主题、来源、出版商、资助者、地理位置和关键词。当搜索学术论文、解析 DOI、下载开放获取 PDF、寻找作者的出版物、汇总文献计量数据（引用次数、h 指数、影响因子）、探索研究分类法或进行 DOI 查找时使用。",
    ...
    if (startIdx !== -1) {
      ...
      console.error('未在 content 中定位 to startMarker');
      process.exit(1);
    }
