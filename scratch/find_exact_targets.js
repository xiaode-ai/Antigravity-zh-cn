  ...
  filesToSearch.forEach(fileObj => {
    ...
    try {
      ...
      while (true) {
        ...
        const start = Math.max(0, foundIndex - 120);
        const end = Math.min(content.length, foundIndex + target.length + 120);
        const context = content.substring(start, end).replace(/\r?\n/g, ' ');
        
        results.push(`[FOUND in ${fileObj.name}] match #${count} at index ${foundIndex}:`);
        results.push(`   CODE: ${content.substring(start, end)}`);
...
fs.writeFileSync('scratch/exact_targets_found.txt', results.join('\n'), 'utf8');
console.log('Search finished. Results written to scratch/exact_targets_found.txt');
