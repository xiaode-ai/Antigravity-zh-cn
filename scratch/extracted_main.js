...
electron_1.app
    ...
    if (deepLinkFromArg) {
        console.log('通过深度链接启动：', deepLinkFromArg);
    ...
    if (DEV_URL) {
        console.log('以开发模式启动，URL：', DEV_URL);
    ...
    if (!fs.existsSync(languageServer_1.LS_BINARY)) {
        ...
        if (HEADLESS) {
            ...
            await electron_1.dialog.showErrorBox('未找到二进制文件', msg);
        }
        electron_1.app.quit();
        return;
    }
