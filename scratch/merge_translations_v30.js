      ...
      if (endSearchIdx !== -1) {
        ...
        if (endLineIdx !== -1) {
          ...
          const replacementHeader = 
            ...
            '    }\\n' +
            // Handle combined file and search counts (e.g. "1 file, 3 searches")
            '    if (/^\\\\d+\\\\s+files?,\\\\s+\\\\d+\\\\s+search(?:es)?$/i.test(normalized)) {\\n' +
            '      const match = normalized.match(/^(\\\\d+)\\\\s+files?,\\\\s+(\\\\d+)\\\\s+search(?:es)?$/i);\\n' +
            '      return leadingSpaces + match[1] + \\\' 个文件，\\\' + match[2] + \\\' 次搜索\\\' + trailingSpaces;\\n' +
            '    }\\n' +
