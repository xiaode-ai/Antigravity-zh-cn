      ...
      if (endSearchIdx !== -1) {
        ...
        if (endLineIdx !== -1) {
          ...
          const replacementHeader = 
            ...
            '    }\\n' +
            '    if (/^Plugin\\\\s*:\\\\s*science$/i.test(normalized)) {\\n' +
            '      return leadingSpaces + \\\'插件：科学\\\' + trailingSpaces;\\n' +
            '    }\\n' +
            '    if (/^Plugin\\\\s*:\\\\s*android-cli-plugin$/i.test(normalized)) {\\n' +
            '      return leadingSpaces + \\\'插件：Android CLI 插件\\\' + trailingSpaces;\\n' +
            '    }\\n' +
