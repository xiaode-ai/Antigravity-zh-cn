...
if (fs.existsSync(nlsPath)) {
  ...
  console.log(`Searching in nls.messages.json (total items: ${nls.length})`);
  nls.forEach((msg, idx) => {
    if (typeof msg === 'string' && (msg.includes('Terminal') || msg.includes('background') || msg.includes('process'))) {
      console.log(`Index ${idx}: "${msg}"`);
    }
  });
}
...
if (fs.existsSync(clpDir)) {
  console.log(`\nSearching in CLP directory: ${clpDir}`);
