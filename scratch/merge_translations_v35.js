...
if (entry) {
  ...
  if (match) {
    ...
    const newTranslations = {
      ...
      "searching": "正在搜索",
      "searching...": "正在搜索...",
      "Messages can be sent while the agent is still working. Your message will be queued and inserted at the next available break in reasoning.": "可以在智能体仍在工作时发送消息。您的消息将被排队，并在下一次推理中断时插入。",
      "Agent terminated due to error": "智能体因错误终止",
      "You can prompt the model to try again or start a new conversation if the error persists.": "如果错误持续存在，您可以提示模型重试或开始新对话。",
      "See our troubleshooting guide for more help.": "请参阅我们的故障排除指南以获取更多帮助。",
      "1 file, 3 searches": "1 个文件，3 次搜索",
      "See our": "请参阅我们的",
      "troubleshooting guide": "故障排除指南",
      "for more help": "以获取更多帮助",
      "for more help.": "以获取更多帮助。",
      "now": "现在",
      "Delete pending message": "删除待处理消息",
      "Proceed": "继续",
      "1 file, 1 folder, 1 search": "1 个文件，1 个文件夹，1 次搜索",
      "11 files, 3 folders, 5 searches": "11 个文件，3 个文件夹，5 次搜索",
      "2 files, 1 folder, 6 searches": "2 个文件，1 个文件夹，6 次搜索",
      "5 files, 3 folders, 5 searches, 10 pages": "5 个文件，3 个文件夹，5 次搜索，10 个页面",
      ...
      "Use when the user asks about pathway analysis, gene list enrichment, retrieving results by token, finding unmapped or not-found identifiers, mapping identifiers, reaction participants (inputs, outputs), pathway hierarchy (including top-level pathways), diagram export, cross-reference mapping, or searching the knowledgebase.": "当用户询问通路分析、基因列表富集、通过 Token 获取结果、寻找未映射或未找到 of 标识符、映射标识符、反应参与者（输入、输出）、通路层级（包括顶级通路）、图表导出、交叉引用映射或搜索知识库时使用。",
    ...
    console.log(`已成功合并所有词条，共添加 ${Object.keys(newTranslations).length} 个键值对！`);
      ...
      if (endSearchIdx !== -1) {
        ...
        if (endLineIdx !== -1) {
          ...
          const replacementHeader = 
            ...
            '      { prefix: \\\'Resolves between rsIDs\\\', trans: \\\'在 rsID、VCF 格式 of 基因组坐标和 HGVS 字符串之间进行解析。对于 rsID，返回变异类型、基因关联、临床意义、等位基因频率和基因组坐标 (GRCh38)。\\\' },\\n' +
            ...
            '      { prefix: \\\'Use for FDA adverse events, recalls\\\', trans: \\\'用于在所有 28 个 API端点上进行 FDA 不良事件、召回、标签、批准、短缺、510(k) 许可、NDC 查找以及任何 FDA 安全或监管数据查询。\\\' },\\n' +
            ...
            '    if (normalized === \\\'Send all\\\') return leadingSpaces + \\\'发送全部\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'searching\\\') return leadingSpaces + \\\'正在搜索\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'searching...\\\') return leadingSpaces + \\\'正在搜索...\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'Agent terminated due to error\\\') return leadingSpaces + \\\'智能体因错误终止\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'You can prompt the model to try again or start a new conversation if the error persists.\\\') return leadingSpaces + \\\'如果错误持续存在，您可以提示模型重试或开始新对话。\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'See our troubleshooting guide for more help.\\\') return leadingSpaces + \\\'请参阅我们的故障排除指南以获取更多帮助。\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'See our\\\') return leadingSpaces + \\\'请参阅我们的\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'troubleshooting guide\\\') return leadingSpaces + \\\'故障排除指南\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'for more help\\\') return leadingSpaces + \\\'以获取更多帮助\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'for more help.\\\') return leadingSpaces + \\\'以获取更多帮助。\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'now\\\') return leadingSpaces + \\\'现在\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'Delete pending message\\\') return leadingSpaces + \\\'删除待处理消息\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'Proceed\\\') return leadingSpaces + \\\'继续\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'This undo action will not make any code changes.\\\') return leadingSpaces + \\\'此撤销操作不会进行任何代码更改。\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'This undo action will not make any code changes\\\') return leadingSpaces + \\\'此撤销操作不会进行任何代码更改\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'Agent execution terminated due to error.\\\') return leadingSpaces + \\\'智能体执行因错误终止。\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'Agent execution terminated due to error\\\') return leadingSpaces + \\\'智能体执行因错误终止\\\' + trailingSpaces;\\n' +
            ...
            '    if (normalized === \\\'Task\\\') return leadingSpaces + \\\'任务\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'task\\\') return leadingSpaces + \\\'任务\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'Task log\\\') return leadingSpaces + \\\'任务日志\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'Task Log\\\') return leadingSpaces + \\\'任务日志\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'task log\\\') return leadingSpaces + \\\'任务日志\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'log\\\') return leadingSpaces + \\\'日志\\\' + trailingSpaces;\\n' +
            '    if (normalized === \\\'Log\\\') return leadingSpaces + \\\'日志\\\' + trailingSpaces;\\n' +
            ...
            '    }\\n' +
            '    if (/^Recent\\\\s+in\\\\s+(.+)$/i.test(normalized)) {\\n' +
            '      const match = normalized.match(/^Recent\\\\s+in\\\\s+(.+)$/i);\\n' +
            '      return leadingSpaces + \\\'最近在 \\\' + match[1].trim() + trailingSpaces;\\n' +
            '    }\\n' +
            '    if (/^Pending\\\\s+messages\\\\s*(.*)$/i.test(normalized)) {\\n' +
            ...
            '    }\\n' +
            '    if (/^Send\\\\s+all\\\\s*\\\\((\\\\d+)\\\\)$/i.test(normalized)) {\\n' +
            '      const match = normalized.match(/^Send\\\\s+all\\\\s*\\\\((\\\\d+)\\\\)$/i);\\n' +
            '      return leadingSpaces + \\\'发送全部 (\\\' + match[1] + \\\')\\\' + trailingSpaces;\\n' +
            '    }\\n' +
            ...
            '    }\\n' +
            // Handle report combinations dynamically (e.g. "1 file, 1 folder, 1 search")
            '    if (normalized.includes(\\\',\\\')) {\\n' +
            '      const parts = normalized.split(\\\',\\\').map(p => p.trim());\\n' +
            '      const isReport = parts.every(p => /^\\\\d+\\\\s+(?:(?:file|folder|task|result|artifact|page)s?|search(?:es)?)$/i.test(p));\\n' +
            '      if (isReport) {\\n' +
            '        const typeMap = {\\n' +
            '          \\\'file\\\': \\\'个文件\\\',\\n' +
            '          \\\'files\\\': \\\'个文件\\\',\\n' +
            ...
            '          const cnUnit = typeMap[unit] || unit;\\n' +
            '          return num + \\\' \\\' + cnUnit;\\n' +
            '        });\\n' +
            '        return leadingSpaces + translatedParts.join(\\\'，\\\') + trailingSpaces;\\n' +
            '      }\\n' +
            '    }\\n' +
            ...
            '    }\\n' +
            '    if (/^\\\\d+\\\\s+pages?$/i.test(normalized)) {\\n' +
            '      return leadingSpaces + normalized.replace(/(\\\\d+)\\\\s+pages?/gi, \\\'$1 个页面\\\') + trailingSpaces;\\n' +
            '    }\\n' +
